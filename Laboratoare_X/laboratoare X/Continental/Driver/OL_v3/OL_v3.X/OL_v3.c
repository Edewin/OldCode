 ...
 * Author: Eduard
 *
 * Created on March 31, 2014, 3:46 PM
 */
...
*************************************************************/
unsigned int BrakeTable[]={0x0000,
                            0x2004, 0x0210, 0x0204,
			    0x0801, 0x2001, 0x0810,
                            0x0000};
	...
	{		
		...
		OVDCON = StateLoTableAntiClk[HallValue];	// Load the overide control register
			...
			/*while(frana)							// if S2 is pressed, stop
				{                                      
                                //    OVDCON = StateLoTableAntiClk[2];	// Load the overide control register
                                 //   DelayNmSec(100);
...
void init_INT(void)
{
   ...
   // TRISDbits.TRISD1=1;
    ...
    IPC5bits.INT2IP=3;  // interrupt priority
